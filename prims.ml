  ...
  let misc_ops =
    let misc_parts = [
        ...
        "STRING_LENGTH rsi, rsi
         MAKE_RATIONAL(rax, rsi, 1)", make_unary, "string_length"; (* first ele is the body, string-len is a label *)
	 ...
	 mov rdx, rax
         ...
         "mov rbx, PVAR(0)
          mov rdx, PVAR(1)
          MAKE_PAIR(rax, rbx, rdx)", make_binary, "cons";
         "mov rax, PVAR(0)
          CAR rax, rax
          leave
          ret", make_unary, "car";
         ...
         "mov rsi, [rbp + 3 * WORD_SIZE]              ;; PARAMS COUNT
          mov rbx, [rbp + (rsi + 3) * WORD_SIZE]      ;; rbx is the last argument of apply
          mov rcx, 0                                 ;; rcx used as a counter of params
          cmp byte [rbx], T_NIL
          je for_both_empty_and_non_empty_list
          mov rdx, rbx                                ;; rdx points to list
          mov rdi, 0                                  ;; rdi used as counter of length
          calc_list_length_loop:
          add rdi, 1                                  ;; rdi++
          CDR rdx, rdx                              ;; rdx point to the rest of the list
          cmp byte [rdx], T_NIL                       ;; checking if the rest is null
          je after_calc_list_length
          cmp byte [rdx], T_PAIR                      ;; if the rest isnt a null, keep counting
          je calc_list_length_loop
          after_calc_list_length:
          shl rdi, 3                                  ;; rdi = length of list * 8
          sub rsp, rdi                                ;; rsp = rsp - rdi 
          push_list_loop:
          CAR rax, rbx                                ;; rax points to the first element in the list
          mov qword [rsp], rax                        ;; 
          add rsp, WORD_SIZE
          add rcx, 1                                  ;; rcx = number of elements in opt
          CDR rbx, rbx
          cmp byte [rbx], T_NIL
          je after_push_list_loop
          jmp push_list_loop
          after_push_list_loop:
          sub rsp, rdi
          for_both_empty_and_non_empty_list:
          sub rsi, 2                                 ;; sub from rsi (which is number of params in stack) proc and opt
          add rcx, rsi
          push_rest_arguments_loop:
          cmp rsi, 0
          je push_rest_loop
          mov rax, [rbp + (WORD_SIZE * (4 + rsi))] 
          push rax
          dec rsi
          jmp push_rest_arguments_loop
          push_rest_loop:
          push rcx
          add rcx, 3
          ...
          mov r13, qword [rbp]                                          ;; old rbp
          SHIFT_FRAME_OF_APPLY rcx
          mov rbp, r13
          jmp rax  
          leave
          ret", make_binary, "apply"
      ] in
